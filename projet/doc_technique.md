...
Le moteur de combat : Détecté via la présence du mot-clé [COMBAT] dans un choix. Gère le tour par tour, les chances de fuite (50%), le calcul des dégâts avec composante aléatoire (Random), l'application des soins et l'attribution de la force bonus en cas de victoire.
...
Intègre la logique métier lors de l'instanciation (Création de compte) : attribution aléatoire de 3 pouvoirs répartis dans 3 catégories de rareté distinctes, avec gestion d'une exception interactive (le pouvoir ID 6 "Au choix" requiert une saisie utilisateur).
